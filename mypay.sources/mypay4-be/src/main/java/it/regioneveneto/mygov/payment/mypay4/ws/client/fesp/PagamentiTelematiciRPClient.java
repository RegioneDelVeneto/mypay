...
public class PagamentiTelematiciRPClient extends BaseClient implements PagamentiTelematiciRP {
  ...
  public ChiediFlussoSPCRisposta chiediFlussoSPC(ChiediFlussoSPC request) {
    return (ChiediFlussoSPCRisposta) getWebServiceTemplate().marshalSendAndReceive(request, getMessageCallback());
  }

  @Override
  public ChiediFlussoSPCPageRisposta chiediFlussoSPCPage(ChiediFlussoSPCPage request){
    return (ChiediFlussoSPCPageRisposta) getWebServiceTemplate().marshalSendAndReceive(request, getMessageCallback());
  }

  @Override
  public ChiediListaFlussiSPCRisposta chiediListaFlussiSPC(ChiediListaFlussiSPC request) {
    return (ChiediListaFlussiSPCRisposta) getWebServiceTemplate().marshalSendAndReceive(request, getMessageCallback());
  }

  @Override
  public NodoSILChiediCopiaEsitoRisposta nodoSILChiediCopiaEsito(NodoSILChiediCopiaEsito request){
    return (NodoSILChiediCopiaEsitoRisposta) getWebServiceTemplate().marshalSendAndReceive(request, getMessageCallback());
  }

  @Override
  public NodoSILInviaRPRisposta nodoSILInviaRP(NodoSILInviaRP request, IntestazionePPT header) {
    return (NodoSILInviaRPRisposta) getWebServiceTemplate().marshalSendAndReceive(request, getMessageCallback(header));
  }

  @Override
  @LogExecution
  public NodoSILChiediIUVRisposta nodoSILChiediIUV(NodoSILChiediIUV request) {
    return (NodoSILChiediIUVRisposta) getWebServiceTemplate().marshalSendAndReceive(request, getMessageCallback());
  }

  @Override
  @LogExecution
  public NodoSILInviaCarrelloRPRisposta nodoSILInviaCarrelloRP(NodoSILInviaCarrelloRP request) {
    return (NodoSILInviaCarrelloRPRisposta) getWebServiceTemplate().marshalSendAndReceive(request, getMessageCallback());
  }

  @Override
  @LogExecution
  public NodoSILRichiediRTRisposta nodoSILRichiediRT(NodoSILRichiediRT request) {
    return (NodoSILRichiediRTRisposta) getWebServiceTemplate().marshalSendAndReceive(request, getMessageCallback());
  }
}
