...
public class RptRtService {
  ...
  // Multi-beneficiary IUV management if defined (IUV_MULTI_08) - Setting the CCP as reported in the documentation
  // The CCP of the RPT must contain the idCarrello. This peculiarity serves to facilitate the association
  // of RTs generated by the PSPs at the same debt position. The idCarrello must be composed in the following form:
  //   <idDominio(11)><numeroAvviso(18)>-(1)<Progressivo(5)>
  // example: "12345678912301123456789034214-00001"
  @Transactional(transactionManager = "tmFesp", propagation = Propagation.REQUIRED)
  public String generateCCP(String idDominio, String iuv){
    String codAvviso;
    if (iuv.length() == Constants.IUV_GENERATOR_17_LENGTH) {
      codAvviso = Constants.SMALL_IUV_AUX_DIGIT + iuv;
    } else if (iuv.length() == Constants.IUV_GENERATOR_15_LENGTH) {
      var ente = enteService.getEnteByCodFiscale(idDominio);
      codAvviso = Constants.OLD_IUV_AUX_DIGIT + ente.getCodCodiceSegregazione() + iuv;
    } else {
      throw new ValidatorException("wrong IUV length!");
    }
    String ccp = null;
    int index = 1;
    boolean idCarrelloAlreadyPresent = true;
    while(idCarrelloAlreadyPresent) {
      ccp = String.format("%s%s-%05d", idDominio, codAvviso, index++);
      idCarrelloAlreadyPresent = self.getRPTByIdDominioIuvCdContestoPagamento(idDominio, iuv, ccp) != null;
    }
    log.debug("ccp multiBeneficiary is: {}", ccp);
    return ccp;
  }
}
