...
public class JAXBTransformService {
  ...
  public <T> T unmarshalling(Document document, Class<T> clazz) {
    return this.unmarshalling(document, clazz, null);
  }
  public <T> T unmarshalling(Document document, Class<T> clazz, String xsdFile) {
    try {
  ...
  public String getDetailUnmarshalExceptionMessage(MyPayException unmarshallingException, byte[] xml){
    String detailErrorMessage = null;
    try {
      if (unmarshallingException.getCause().getCause() instanceof SAXParseException) {
        SAXParseException saxExc = (SAXParseException) unmarshallingException.getCause().getCause();
    ...
    if(StringUtils.isBlank(detailErrorMessage))
      detailErrorMessage = unmarshallingException.getMessage();
