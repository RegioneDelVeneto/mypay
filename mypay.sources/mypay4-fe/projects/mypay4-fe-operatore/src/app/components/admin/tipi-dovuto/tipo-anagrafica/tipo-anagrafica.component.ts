...
export class TipoAnagraficaComponent implements OnInit, OnDestroy {
  ...
  flgScadenzaObbligatoriaEnable = false;
  ...
  private setForm(anagTipoDovuto: AnagraficaTipoDovuto) {
    this.flgScadenzaObbligatoriaEnable = anagTipoDovuto.flgScadenzaObbligatoriaEnable;
    ...
    this.formAnag.get('flgScadenzaObbligatoria').setValue(
      this.flgScadenzaObbligatoriaEnable ? anagTipoDovuto.flgScadenzaObbligatoria : false //Must have been already false.
    );
    this.formAnag.get('flgStampaDataScadenza').setValue(
      this. flgScadenzaObbligatoriaEnable ? anagTipoDovuto.flgStampaDataScadenza : false //Must have been already false.
    );
    ...
    this.formAnag.get('codServiceAccountJwtUscitaSecretKey').setValue(anagTipoDovuto.codServiceAccountJwtUscitaSecretKey);
        ...
        if (anagTipoDovuto.macroArea) {
          let codTipoEnte = this.ente.codTipoEnte;
          let macroArea = this.macroAreas.filter(m => m.code == anagTipoDovuto.macroArea)[0];
          ...
          this.adminEntiService.getTipoServizio(codTipoEnte, macroArea.code).subscribe(
            tipoServizios => {
              ...
              if (anagTipoDovuto.tipoServizio) {
                let tipoServizio = tipoServizios.filter(t => t.code == anagTipoDovuto.tipoServizio)[0];
                ...
                this.adminEntiService.getMotivoRiscossione(codTipoEnte, anagTipoDovuto.macroArea, anagTipoDovuto.tipoServizio).subscribe(
                  motivoRiscossiones => {
                    ...
                    if (anagTipoDovuto.motivoRiscossione) {
                      let motivoRiscossione = motivoRiscossiones.filter(m => m.code == anagTipoDovuto.motivoRiscossione)[0];
                      ...
                      this.adminEntiService.getCodTassonomico(codTipoEnte,  anagTipoDovuto.macroArea, anagTipoDovuto.tipoServizio, anagTipoDovuto.motivoRiscossione).subscribe(
                        codTassonomicos => {
                          ...
                          if (anagTipoDovuto.codTassonomico) {
                            this.formAnag.get('codTassonomico').setValue(
                              codTassonomicos.find(c => c.code == anagTipoDovuto.codTassonomico) );
                          } else {
  ...
  spontaneoOnChange(isSpontaneo: boolean) {
    ...
    this.flgScadenzaObbligatoriaEnable = !isSpontaneo;
  }

  flgScadenzaObbligatoriaOnChange(flgDataScadenza: boolean, alertTemplateRef) {
    if (flgDataScadenza) {
  ...
  private setMode(mode: string) {
    ...
    if (mode === 'edit') {
      ...
      if (!this.formAnag.get('spontaneo').value) {
        if(this.flgScadenzaObbligatoriaEnable) {
          this.formAnag.get('flgScadenzaObbligatoria').enable();
